<<<<<<< HEAD
// Vector class to handle vector operations like addition, subtraction, and scaling
class Vector {
  constructor(x, y) {
    this.x = x || 0;
    this.y = y || 0;
...
// Mouse class to track mouse position
class Mouse {
  constructor(canvas) {
...
// Dot class for each point in the rope
class Dot {
  constructor(x, y) {
...
// Stick class to create a stick between two dots
class Stick {
  constructor(p1, p2) {
    this.startPoint = p1;
    this.endPoint = p2;
...
// Rope class that combines dots and sticks
class Rope {
  constructor(config) {
    this.x = config.x;
    this.y = config.y;
...
// Main App class that runs the animation
class App {
  ...
  render() {
    ...
    const frame = () => {
      ...
      this.ropes.forEach(rope => rope.update(this.mouse));
